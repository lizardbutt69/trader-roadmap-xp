    ...
    >
      <Link to={`/blog/${post.slug}`} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
        ...
        >
          ...
          {/* Title */}
          <h2 style={{
            fontSize: 19, fontWeight: 600, lineHeight: 1.25, letterSpacing: '-0.02em',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {(post.tags || []).slice(0, 2).map(tag => (
                <span key={tag} style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', fontWeight: 600 }}>
                  #{tag}
                </span>
              ))}
            </div>
    ...
    <Link to={`/blog/${post.slug}`} style={{ textDecoration: 'none', display: 'block', marginBottom: 32 }}>
      ...
      >
        ...
        <div style={{ position: 'relative' }}>
          ...
          <h2 style={{
            fontSize: 26, fontWeight: 600, lineHeight: 1.2, letterSpacing: '-0.03em',
    ...
    <div style={{ minHeight: '100vh', background: '#0b0d13', color: '#eaebf0', fontFamily: "'Plus Jakarta Sans', sans-serif", overflowX: 'hidden' }}>
      ...
      <style>{`
        ...
        @media (max-width: 680px) {
          ...
          .blog-hero-section { padding: 80px 20px 40px !important; }
        }
      `}</style>
      ...
      <nav style={{
        ...
        <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
          <TradeSharpLogo size={30} />
          <span style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-0.02em', color: '#eaebf0' }}>
            Trade<span style={{ color: '#22d3ee' }}>Sharp</span>
          </span>
        </Link>

        <div className="blog-nav-links" style={{ alignItems: 'center', gap: 28 }}>
          <Link to="/" style={{ fontSize: 14, fontWeight: 500, color: 'rgba(255,255,255,0.45)', textDecoration: 'none', transition: 'color 0.15s' }}
            onMouseEnter={e => e.target.style.color = '#eaebf0'} onMouseLeave={e => e.target.style.color = 'rgba(255,255,255,0.45)'}>Home</Link>
          <span style={{ fontSize: 14, fontWeight: 600, color: '#22d3ee' }}>Blog</span>
        </div>

        <div className="blog-ctas" style={{ alignItems: 'center', gap: 10 }}>
          <button onClick={() => navigate('/login')} style={{
            padding: '8px 18px', borderRadius: 7, background: 'transparent',
          ...
          <button onClick={() => navigate('/login')} style={{
            ...
            boxShadow: '0 0 18px rgba(34,211,238,0.2)', transition: 'box-shadow 0.15s',
          }}
            onMouseEnter={e => e.currentTarget.style.boxShadow = '0 0 28px rgba(34,211,238,0.35)'}
            onMouseLeave={e => e.currentTarget.style.boxShadow = '0 0 18px rgba(34,211,238,0.2)'}
      ...
      <main style={{ position: 'relative', zIndex: 1 }}>
        {/* Hero */}
        <section className="blog-hero-section" style={{ padding: '80px 48px 48px', maxWidth: 1100, margin: '0 auto' }}>
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <div style={{
              ...
              <span style={{ fontSize: 10, fontWeight: 700, color: '#22d3ee', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                TradeSharp Blog
              </span>
            </div>
